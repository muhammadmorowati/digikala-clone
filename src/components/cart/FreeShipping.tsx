...
export default function FreeShipping() {
  ...
  const provinceTitle =
    userCity &&
    province.find((item) => item.id === userCity.province_id)?.title;
  ...
  return (
    <>
        ...
        <div className="flex items-center gap-2">
          ...
          <div>
            ...
            <p className="text-[11px] text-neutral-500">
              ...
              {provinceTitle && (
                <span className="text-xs text-red-500">{provinceTitle}</span>
              )}
            </p>
          </div>
        </div>
      ...
      {/* Modal */}
      <div
        className={`fixed right-0 bottom-0 z-50 flex h-screen w-full cursor-default
          items-center bg-black/40 dark:bg-black/80 transition-all duration-500
      ...
      >
        <div
          ...
          className={`fixed right-0 left-0 lg:top-0 bottom-0 lg:m-auto max-lg:w-full 
            max-w-xl bg-white dark:bg-neutral-700 px-5 pb-5 pt-0 shadow 
