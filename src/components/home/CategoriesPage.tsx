...
export default function CategoriesPage({
  ...
  return (
    <div className="lg:mx-3 my-12">
      <h2 className="text-center text-xl font-irsansb my-2">
        خرید بر اساس دسته‌بندی
      </h2>

      {chunks.map((chunk, idx) => (
        <Swiper key={idx} className="mt-10" breakpoints={breakpoints}>
          {chunk.map((category) => (
            <SwiperSlide
              key={category._id.toString()}
              className="flex flex-col gap-5"
            >
              <Link href={category.href} className="flex items-center flex-col">
                <Image
                  alt={category.title}
                  width={100}
                  height={100}
                  src={category.cover?.[0] ?? "/placeholder-category.png"}
