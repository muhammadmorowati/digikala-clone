...
import { ReturnQuestion } from "@/src/utils/types"; // ✅ add type safety
import { returnQuestions } from "@/src/data/data"; // ✅ use dataset specific to returns

interface ReturnAccordionProps {
  data?: ReturnQuestion[]; // ✅ allows passing dynamic data
}

export default function ReturnAccordion({ data = returnQuestions }: ReturnAccordionProps) {
  if (!data?.length) {
    return (
